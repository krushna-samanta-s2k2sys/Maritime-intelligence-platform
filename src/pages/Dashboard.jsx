...
const KPIS = [
  { v:'847,392', l:'Total Vessels',       delta:'+1,243 this month', up:true,  color:'#1558d6' },
  { v:'94.3%',   l:'Active Fleet',        delta:'−0.4% vs last month', up:false, color:'#137333' },
  { v:'9,241',   l:'Ports Tracked',       delta:'+18 new ports', up:true,  color:'#0094b3' },
  { v:'283',     l:'PSC Detentions YTD',  delta:'+12 vs same period', up:false, color:'#c8102e' },
  { v:'1,047',   l:'Certs Expiring (30d)',delta:'247 critical (<7d)', up:false, color:'#b45309' },
  { v:'62',      l:'Active Sanctions',    delta:'+4 new designations', up:false, color:'#6200ea' },
  { v:'42,881',  l:'Companies',           delta:'+312 this quarter', up:true,  color:'#ea580c' },
  { v:'94.2B',   l:'AIS Points (total)',  delta:'+1.4B this month', up:true,  color:'#137333' },
]

const FLEET_TYPES = [
  ['Bulk Carrier',       210841, '#1558d6'],
  ['General Cargo',      198422, '#0094b3'],
  ['Oil Tanker',          96384, '#c8102e'],
  ['Container Ship',      55912, '#137333'],
  ['Chemical Tanker',     48271, '#b45309'],
  ['Offshore Supply',     37885, '#6200ea'],
  ['LNG / LPG Carrier',  14022, '#ea580c'],
  ['Passenger / Cruise', 11244, '#d93025'],
  ['RoRo / Car Carrier',  9812, '#717a85'],
  ['Other',             164599, '#aab2bd'],
]
...
const VESSEL_GEO = {
  ...
  features:[
    ...
    {type:'Feature',geometry:{type:'Point',coordinates:[55.0,22.0]},   properties:{name:'EURONAV NINA',    type:'Oil Tanker',       speed:13.2,flag:'BE',status:'Underway', imo:'9320116'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[45.0,11.0]},   properties:{name:'PIONEER MAX',     type:'LPG Carrier',      speed:11.8,flag:'MH',status:'Underway', imo:'9612988'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[19.0,36.5]},   properties:{name:'ADRIATIC SPIRIT', type:'Car Carrier',      speed:17.2,flag:'GR',status:'Underway', imo:'9445677'}},
    ...
    {type:'Feature',geometry:{type:'Point',coordinates:[14.0,55.5]},   properties:{name:'DIANA BULKER',    type:'Bulk Carrier',     speed:10.1,flag:'MH',status:'Underway', imo:'9501882'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[10.0,57.5]},   properties:{name:'BRAVE TERN',      type:'Offshore Wind',    speed:8.4, flag:'NO',status:'Underway', imo:'9593513'}},
    ...
    {type:'Feature',geometry:{type:'Point',coordinates:[50.0,26.5]},   properties:{name:'BOURBON LIBERTY', type:'Offshore Supply',  speed:9.2, flag:'FR',status:'Underway', imo:'9450993'}},
...
const ROUTE_GEO = {
  type:'FeatureCollection',
  features:[
    {type:'Feature',properties:{name:'Asia–Europe (Suez)',trade:'container'},geometry:{type:'LineString',coordinates:[[121.5,31.2],[110,22],[103.8,1.3],[95,6],[80.5,8],[70,12],[55,12],[44,12.5],[37.5,13.5],[42,15.5],[43,23.5],[32.5,29.9],[32.5,31.5],[28,34.5],[20,36],[13,38],[5,44],[-2,48],[4.2,51.9]]}},
    {type:'Feature',properties:{name:'Trans-Pacific',trade:'container'},geometry:{type:'LineString',coordinates:[[121.5,25],[135,30],[150,35],[170,35.5],[-170,35],[-155,30],[-140,25],[-125,23],[-120,25],[-118.5,34]]}},
    {type:'Feature',properties:{name:'Asia–Europe (Cape)',trade:'tanker'},geometry:{type:'LineString',coordinates:[[103.8,1.3],[95,-5],[85,-14],[68,-20],[52,-25],[22,-34],[10,-30],[-5,-20],[-15,-10],[-10,5],[-5,15],[-8,35],[-5,43],[4.2,51.9]]}},
    {type:'Feature',properties:{name:'Trans-Atlantic',trade:'container'},geometry:{type:'LineString',coordinates:[[4.2,51.9],[-10,48],[-25,47],[-40,44],[-55,42],[-65,38],[-74,40.7]]}},
    {type:'Feature',properties:{name:'Persian Gulf–Asia',trade:'tanker'},geometry:{type:'LineString',coordinates:[[50,26.5],[57,22],[62,18],[72,10],[78,5],[88,5],[103.8,1.3],[121.5,25]]}},
    {type:'Feature',properties:{name:'Australia–Asia',trade:'bulk'},geometry:{type:'LineString',coordinates:[[115.9,-32],[124,-24],[130,-15],[125,-8.5],[120,5],[110,18],[121.5,25]]}},
  ]
}
...
const PORT_GEO = {
  ...
  features:[
    ...
    {type:'Feature',geometry:{type:'Point',coordinates:[-95,29.7]},    properties:{name:'Houston',         calls:12441,type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[4.3,51.2]},    properties:{name:'Antwerp',         calls:11882,type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[129,35.1]},    properties:{name:'Busan',           calls:10442,type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[114.2,22.3]},  properties:{name:'Hong Kong',       calls:9882, type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[120.3,36.1]},  properties:{name:'Qingdao',         calls:8441, type:'secondary'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[10,53.5]},     properties:{name:'Hamburg',         calls:12441,type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[30,31.2]},     properties:{name:'Port Said',       calls:6882, type:'secondary'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[32.5,29.9]},   properties:{name:'Suez',            calls:19441,type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[-74,40.7]},    properties:{name:'New York/NJ',     calls:8882, type:'major'}},
    {type:'Feature',geometry:{type:'Point',coordinates:[115.8,-31.9]}, properties:{name:'Fremantle',       calls:2441, type:'secondary'}},
  ]
}
...
const CHOKE_POINTS = [
  ...
  { latlng:[40.0,29.0],  name:'Bosphorus',  stat:'48k vessels/yr' },
  { latlng:[12.6,43.5],  name:'Bab-el-Mandeb', stat:'6.5Mb oil/day' },
]
...
function vesselColor(p) {
  ...
  const m = {'Container Ship':'#1558d6','Oil Tanker':'#c8102e','LNG Carrier':'#ea580c',
    ...
    'Research Vessel':'#059669','RoRo':'#b45309','Offshore Wind':'#0891b2',
    'General Cargo':'#64748b'}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setActivity(shuffled(ACT_POOL).slice(0, 14)), 30000)
    return () => clearInterval(interval)
  }, [])

  useEffect(() => {
    if (mapRef.current) return
    const map = L.map(mapElRef.current, { center:[20,20], zoom:2, zoomControl:true, attributionControl:false })
    mapRef.current = map
    ...
    const routeLayer = L.geoJSON(ROUTE_GEO, {
      ...
      onEachFeature: (f, layer) => layer.bindTooltip(f.properties.name, { className:'chokeLabel', permanent:false })
    }).addTo(map)
    ...
    const portLayer = L.geoJSON(PORT_GEO, {
      ...
      onEachFeature: (f, layer) => layer.bindPopup(
        ...
        `<div class="vpop-row"><span class="vpop-lbl">Port Calls (annual)</span><span class="vpop-val">${f.properties.calls.toLocaleString()}</span></div>` +
        `<div class="vpop-row"><span class="vpop-lbl">Category</span><span class="vpop-val">${f.properties.type.toUpperCase()}</span></div>` +
        `<a href="#" class="vpop-link" onclick="event.preventDefault()">View Port Details →</a></div>`
      )
    }).addTo(map)
    ...
    const vesselLayer = L.geoJSON(VESSEL_GEO, {
      ...
      onEachFeature: (f, layer) => {
        ...
        layer.bindPopup(
          ...
          `<div class="vpop-row"><span class="vpop-lbl">Flag</span><span class="vpop-val">${p.flag}</span></div>` +
    ...
    CHOKE_POINTS.forEach(c => {
      L.marker(c.latlng, {
        icon: L.divIcon({ className:'', html:`<div class="chokeLabel">${c.name}</div>`, iconAnchor:[0,0] })
  ...
  function toggleLayer(name) {
    ...
    setLayersOn(prev => {
      ...
      if (next[name]) map.addLayer(lyr)
      else map.removeLayer(lyr)
      return next
    })
  }

  const maxFleet = FLEET_TYPES[0][1]
  const maxFlag  = FLAGS[0][2]
  ...
  return (
    <div className="main">
      ...
      {/* MAP + ACTIVITY */}
      <div className="row2" style={{flexShrink:0}}>
        <div className="panel" style={{minHeight:390}}>
          <div className="panelH">
            ...
            <span style={{marginLeft:'auto',fontSize:10,color:'var(--txt3)'}}>
              {VESSEL_GEO.features.length} sample vessels shown (25 of 847,392)
            </span>
            <button className="btn btnS btnSm" style={{marginLeft:6}} onClick={() => navigate('/gis-ais')}>Full GIS →</button>
          </div>
          <div style={{position:'relative',flex:1}}>
            <div ref={mapElRef} id="dashMap" style={{height:340,borderRadius:'0 0 6px 6px'}} />
            ...
            <div className="mapOverlay mapLayerCtl" style={{top:8,right:8,pointerEvents:'all'}}>
              {['vessels','routes','ports','choke'].map(name => (
                <button key={name} className={`mapLayerBtn${layersOn[name]?' on':''}`} onClick={() => toggleLayer(name)}>
                  {name==='vessels'?'🚢 Vessels':name==='routes'?'〰 Routes':name==='ports'?'⚓ Ports':'⚠ Choke'}
                </button>
              ))}
            </div>
          </div>
      ...
      {/* CHARTS ROW */}
      <div className="row3" style={{flexShrink:0}}>
        <div className="panel">
          <div className="panelH">
          ...
          <div className="panelB">
            {FLEET_TYPES.map(([label, count, color]) => (
              <div key={label} className="barRow">
                <div className="barL" style={{width:110,flexShrink:0,color:'var(--txt2)',fontSize:10,textAlign:'right',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{label}</div>
                <div className="barTr" style={{flex:1,background:'var(--bg3)',borderRadius:2,height:14,overflow:'hidden'}}>
                  <div className="barFl" style={{width:`${Math.round(count/maxFleet*100)}%`,background:color,height:'100%',borderRadius:2}} />
                </div>
          ...
          <div style={{flex:1,overflowY:'auto'}}>
            {FLAGS.map(([flag, name, count]) => (
              <div key={name} className="fRow">
                ...
                <div className="fBar"><div className="fBarFl" style={{width:`${Math.round(count/maxFlag*100)}%`}} /></div>
      ...
      {/* PSC + MARKET ROW */}
      <div className="row2b" style={{flexShrink:0}}>
        <div className="panel">
          <div className="panelH">
          ...
          <div className="tWrap">
            <table className="dt">
              <thead><tr>
                <th>Vessel</th><th>Port</th><th>MOU Region</th><th>Deficiencies</th><th>Date</th><th>Status</th>
              </tr></thead>
              <tbody>
                {PSC.map((p, i) => (
                  <tr key={i}>
