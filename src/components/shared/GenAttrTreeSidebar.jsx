...
export default function GenAttrTreeSidebar({ tree, activeNode, onSelectNode, favorites = new Set(), onToggleFavorite, personaAttrSections }) {
  ...
  const sorted = sortedTopLevel(tree, favorites, personaAttrSections)
  ...
  if (collapsed) {
    return (
      <div className="atSidebar atSbCollapsed">
        <button className="atSbExpandBtn" onClick={() => setCollapsed(false)} title="Expand navigation">›</button>
      </div>
    )
  }

  return (
    <div className="atSidebar" style={{ flex: `0 0 ${sbWidth}px`, width: sbWidth, minWidth: sbWidth }}>
      <div className="atSbHeader">
        <span className="atSbHeaderTitle">Navigation</span>
        <button className="atSbCollapseBtn" onClick={() => setCollapsed(true)} title="Collapse navigation">‹</button>
      </div>
